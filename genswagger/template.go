...
func renderServices(pkg string, services []*descriptor.Service, paths swaggerPathsObject, reg *descriptor.Registry, requestResponseRefs, customRefs refMap) error {
...
func applyTemplate(p param) (*swaggerObject, error) {
	...
	if err := renderServices(p.GetPackage(), p.Services, s.Paths, p.reg, requestResponseRefs, customRefs); err != nil {
		panic(err)
	}
